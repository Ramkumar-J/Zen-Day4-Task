...
let foods=["Biriyani","Idly","Dosa","Chappathi","Pongal","Puri","Coconut rise","Tomato rise","Lemon rise","Fish fry","Chicken65","Parotta","Meals","Fried rise","Noodles","Curd rise","Pizza","Fruit salad","Idiyaapam","Daal rise"];
console.log(foods[4]);
...
var friendsLength = friends.map((word) => {
 ...
 const totalsum = friendsLength.reduce((previousValue, currentValue) => previousValue + currentValue, 0 );
...
var maximum = Math.max.apply(Math, friends.map(function(str) { return str.length; }));
console.log(maximum);

// 9.Find the name with min characters and return the name.
var minimum = Math.min.apply(Math, friends.map(function(str) { return str.length; }));
console.log(minimum);
    }
...
const findNum = friendsInfo.filter(
    element => typeof element === 'number'
  );
  console.log(findNum);
const sumofNum =findNum .reduce((previousValue, currentValue) => previousValue + currentValue, 0 );
